package com.runye.express.activity.app;
...
public class MyApplication extends Application {
	...
	private void initEMChat() {
		...
		options.setNotificationEnable(PreferenceUtil.getInstance(applicationContext).getSettingMsgNotification());
		// 设置收到消息是否有声音提示，默认为true
		options.setNoticeBySound(PreferenceUtil.getInstance(applicationContext).getSettingMsgSound());
		// 设置收到消息是否震动 默认为true
		options.setNoticedByVibrate(PreferenceUtil.getInstance(applicationContext).getSettingMsgVibrate());
		// 设置语音消息播放是否设置为扬声器播放 默认为true
		options.setUseSpeaker(PreferenceUtil.getInstance(applicationContext).getSettingMsgSpeaker());
