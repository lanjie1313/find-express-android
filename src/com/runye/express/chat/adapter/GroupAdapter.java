...
public class GroupAdapter extends ArrayAdapter<EMGroup> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (getItemViewType(position) == 0) {
			if (convertView == null) {
				convertView = inflater.inflate(R.layout.chat_search_bar_with_padding, null);
			}
			...
			if (convertView == null) {
				convertView = inflater.inflate(R.layout.chat_row_add_group, null);
			}
			((ImageView) convertView.findViewById(R.id.avatar)).setImageResource(R.drawable.chat_roominfo_add_btn);
			((TextView) convertView.findViewById(R.id.name)).setText("新建群组");
		} else {
			if (convertView == null) {
				convertView = inflater.inflate(R.layout.chat_row_group, null);
			}
